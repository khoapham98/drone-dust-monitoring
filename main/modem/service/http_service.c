...
eModemResult httpStartService(void)
{
    ...
    if (at_send_wait(AT_CMD_HTTP_START, resp, sizeof(resp), "OK", "ERROR", 500) < 0)
        return WAIT;
...
eModemResult httpStopService(void)
{
    ...
    if (at_send_wait(AT_CMD_HTTP_STOP, resp, sizeof(resp), "OK", "ERROR", 500) < 0)
        return WAIT;
...
eModemResult httpSendData(char* data, int len, int time)
{
    ...
    if (at_send_wait(cmd, resp, sizeof(resp), "DOWNLOAD", NULL, 500) < 0)
        return WAIT;
    ...
    if (at_send_wait(data, resp, sizeof(resp), "OK", "ERROR", 500) < 0)
        return WAIT;
